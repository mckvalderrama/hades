    ...
    <script type="text/javascript">
     {{-- crear recinto--}}
      $(document).on('click','.create-modal', function() {
        ...
        $('.modal-recinto').text('Añadir recinto');
      });
      $("#add").click(function() {
        $.ajax({
          type: 'POST',
          ...
          data: {
            ...
            'status_id': $('option[name=status_id]').val()
          ...
          success: function(data){
            if ((data.errors)) {
              ...
              $('.error').boolean(data.errors.activo);
              
            } else {
              $('.error').remove();
              $('#table').append("<tr class='recinto" + data.id + "'>"+
                "<td>" + data.id + "</td>"+
                "<td>" + data.nombre + "</td>"+
                "<td>" + data.activo + "</td>"+
                "<td><button class='show-modal btn btn-info btn-sm' data-id='" + data.id + "' data-nombre='" + data.nombre + "' data-activo='" + data.activo + "' data-status_id='" + data.status_id + "'><span class='fa fa-eye'></span></button> <button class='edit-modal btn btn-warning btn-sm' data-id='" + data.id + "' data-nombre='" + data.nombre + "' data-activo='" + data.activo + "' data-status_id='" + data.status_id + "'><span class='glyphicon glyphicon-pencil'></span></button> <button class='delete-modal btn btn-danger btn-sm' data-id='" + data.id + "' data-nombre='" + data.nombre + "' data-activo='" + data.activo + "' data-status_id='" + data.status_id + "'><span class='glyphicon glyphicon-trash'></span></button></td>"+
                "</tr>");
            }
          },
        });
     ...
     $(document).on('click', '.edit-modal', function() {
      $('#footer_action_button').text(" Actualizar Recinto");
      $('#footer_action_button').addClass('glyphicon-check');
      $('#footer_action_button').removeClass('glyphicon-trash');
      $('.actionBtn').addClass('btn-success');
      $('.actionBtn').removeClass('btn-danger');
      $('.actionBtn').addClass('edit');
      $('.modal-recinto').text('Editar recinto');
      $('.deleteContent').hide();
      $('.form-horizontal').show();
      $('#fid').val($(this).data('id'));
      $('#n').val($(this).data('nombre'));
      $('#a').val($(this).data('activo'));
      $('#s').val($(this).data('status_id'));
      $('#myModal').modal('show');
    });
     ...
     $('.modal-footer').on('click', '.edit', function() {
      $.ajax({
        ...
        success: function(data) {
          $('.recinto' + data.id).replaceWith(" "+
            "<tr class='recinto" + data.id + "'>"+
            "<td>" + data.id + "</td>"+
            "<td>" + data.nombre + "</td>"+
            "<td>" + data.activo + "</td>"+
...
$(document).on('click', '.delete-modal', function() {
  ...
  $('.modal-recinto').text('Eliminar Recinto');
...
$('.modal-footer').on('click', '.delete', function(){
  ...
  $(document).on('click', '.show-modal', function() {
    $('#show').modal('show');
    $('#idd').text($(this).data('id'));
    $('#no').text($(this).data('nombre'));
    $('#ac').text($(this).data('activo'));
    $('.modal-title').text('Mostrar Recinto');
  });
